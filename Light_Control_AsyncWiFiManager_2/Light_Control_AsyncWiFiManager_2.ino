...
String tankstatus="not available";
...
void setup() 
{
  ...
  if(!ESP_wifiManager.autoConnect("AUTO", "123456789")){
   ...
   server.on("/", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    server.on("/tank", HTTP_GET, [] (AsyncWebServerRequest *request) {
        ...
        if (request->hasParam("value")) {
            message = "Blue_tank_level: ";
            message += request->getParam("value")->value();
        } else {
            message = "No message sent";
        }
        
        if (request->hasParam("Isblacktankfull")) {
            message += ", black_tank_value: ";
            message += request->getParam("Isblacktankfull")->value();
        } else {
            message = "No message sent";
        }
...
void loop()
{
  ...
  //check if wifi is switched on continously
